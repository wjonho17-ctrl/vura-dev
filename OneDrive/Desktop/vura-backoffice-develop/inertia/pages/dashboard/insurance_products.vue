...
const { qs, applyFilter, handlePageChange, isFiltering } = useFiltering<{ name?: string }>()
            ...
            <DataTable ref="dt" filter-display="row" :value="products.data" dataKey="id" :paginator="true"
                ...
                <template #header>
                    <div class="flex flex-wrap items-center gap-2 justify-between">
                        <h4 class="m-0">Insurance Products</h4>
                        <div class="flex gap-4 items-center">
                            <SearchInput :loading="isFiltering" @input="applyFilter" v-model="qs.name"></SearchInput>
                        </div>
                    </div>
                </template>
                ...
                <Column header="Drug Code" field="insuranceDrugCode">
                </Column>

                <Column header="Designation" field="brandName">
                    <template #body="{ data }">
                        <div class="flex flex-col">
                            <span>{{ data.insuranceInfo?.data?.designation }}</span>
                            <span class="text-sm">{{ data.brandName }}</span>
                        </div>
                    </template>
                </Column>

                <Column header="Generic Description" field="genericDescription">
                    <template #body="{ data }">
                        <div class="grid grid-rows-2">
                            <span>{{ data.insuranceInfo?.data?.genericDescription }}</span>
                            <span class="text-sm">{{ data.composition }}</span>
                        </div>
                    </template>
                </Column>

                <Column header="Instructions" field="insuranceInfo.data.instructions"></Column>


                <Column header="Insurance">
                    <template #body="{ data }">
                        <template v-if="data.insurances && data.insurances?.length > 1">
                            <span class="flex justify-center" v-if="isInsuranceProductAsSamePrice(data.insurances)">RWF
                                {{ data.insurances[0].price }}</span>
                ...
                <template #paginatorcontainer>
                    <Pagination @page-change="handlePageChange" :meta="products.meta"></Pagination>
                </template>
