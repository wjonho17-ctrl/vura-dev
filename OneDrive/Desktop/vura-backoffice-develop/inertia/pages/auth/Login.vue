        ...
        <div class="flex flex-col items-center justify-center">
            <div
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20! px-8! sm:px-20!" style="border-radius: 53px">
                    ...
                    <InertiaFrom disableWhileProcessing :action="tuyau.$route('auth.login').path" method="post" 
                        #default="{ errors, processing }">
                        <Message size="large" class="my-2" v-if="errors.message" icon="pi pi-info" severity="error">{{ errors.message }}</Message>
                        <label for="email"
                            class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email</label>
                        <InputText id="email" name="email" type="text" placeholder="Email address" class="w-full md:w-120 mb-2" />
                        <Message variant="simple" severity="error" v-if="errors.email">{{ errors.email }}</Message>

                        <label for="password"
                            class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2! mt-4!">Password</label>
                        <Password id="password" name="password" placeholder="Password" :toggleMask="true"
                            class="mb-2" fluid :feedback="false"></Password>
                        <Message variant="simple" text severity="error" v-if="errors.password">{{ errors.password }}</Message>

                        <div class="flex items-center justify-between mt-6! mb-8! gap-8">
                            <div class="flex items-center">
                                <Checkbox  name="checked" id="checked" binary class="mr-2!"></Checkbox>
                        ...
                        <Button type="submit" :loading="processing" label="Sign In" class="w-full"></Button>
